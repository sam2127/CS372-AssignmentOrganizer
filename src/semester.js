...
function readSettings() {
  try {
    ...
    if (error.code === "ENOENT")
      dialog.showErrorBox("Settings file does not exist.");
    else dialog.showErrorBox("Error!", "An unknown error occurred.");
  }
}
  ...
  try {
    ...
    existingSemesters.forEach((yearObj) => {
      yearObj.semesters.forEach((semester) => {
        if (yearObj.year === year && semester.season === season) {
          dialog.showErrorBox(
            "Error!",
            semester.season + " " + yearObj.year + " semester already exists."
          );
          semesterExists = true;
        }
      });
    ...
    if (!semesterExists) {
      ...
      try {
        ...
        dialog.showErrorBox(
          "Error while saving the semester!",
          "Unknown error."
        );
      }
    }
    ...
    if (error.code === "ENOENT")
      ...
      dialog.showErrorBox(
        "Error!",
        "Unknown error while reading semester data file."
      );
  }
